...
import type { Token, PerformanceMetrics, ThemeColors, WSMessage, FilterState } from './types';
...
const getSavedFilters = (): FilterState => {
  const saved = localStorage.getItem('tokenListFilters');
  if (saved) {
    try {
...
const App: Component = () => {
  ...
  // Add state for token list controls
  const [expandedTokens, setExpandedTokens] = createSignal<Set<string>>(new Set());
  const [isDynamicScaling, setIsDynamicScaling] = createSignal(localStorage.getItem('chartDynamicScaling') === 'true');

  // Save dynamic scaling state when it changes
  createEffect(() => {
    localStorage.setItem('chartDynamicScaling', isDynamicScaling().toString());
  });

  // Handle expand/collapse all
  const handleToggleExpandAll = () => {
    const currentTokens = Object.values(tokens.items);
  ...
  const updateFilters = (newFilters: FilterState | ((prev: FilterState) => FilterState)) => {
    setFilters(prev => {
      ...
      localStorage.setItem('tokenListFilters', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  return (
    ...
    >
      <LiveStatusBar
        ...
        totalTokens={Object.keys(tokens.items).length}
        filteredTokens={Object.keys(tokens.items).length}
        expandedTokens={expandedTokens().size}
        onToggleExpandAll={handleToggleExpandAll}
        isAnyExpanded={expandedTokens().size > 0}
        onDownloadLogs={handleDownloadLogs}
        isDynamicScaling={isDynamicScaling()}
        onToggleDynamicScaling={() => setIsDynamicScaling(prev => !prev)}
        filters={filters()}
      ...
      <main class="flex-1 overflow-auto relative w-full">
        ...
        >
          <TokenEventsList
            ...
            onExpandedChange={setExpandedTokens}
            isDynamicScaling={isDynamicScaling()}
            filters={filters()}
            onUpdateFilters={updateFilters}
          />
